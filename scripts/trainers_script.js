...
function onFormSubmit() {
    var trn = { fname: '', lname: '', courses: {} };
    
    trn['fname'] = document.getElementById('fname').value;
    trn['lname'] = document.getElementById('lname').value;
...
function insertData(trn) {
    //Responds to the table that will be filled.
    const table = document.querySelector(".trainers-table");
    ...
    cell0.innerText = trn['fname'];
    cell1.innerText = trn['lname'];
...
function onEdit(td) {
    ...
    let tempTrn = trainers[tempIndex];
    ...
    document.getElementById('editFname').value = tempTrn['fname'];
    document.getElementById('editLname').value = tempTrn['lname'];
...
function onSave(td) {
    ...
    tempTrn['fname'] = selectedRow.cells[0].innerText;
    tempTrn['lname'] = selectedRow.cells[1].innerText;
...
function onDelete(td) { 
    ...
    if (confirm("This course will be deleted.")){
        ...
        document.querySelector(".trainers-table").deleteRow(selectedRow.rowIndex);
        //Removes the corresponding object from the courses array.
        trainers.splice(tempIndex, 1);
    }
    //Checking
    console.log(trainers);
}